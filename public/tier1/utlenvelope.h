...
public:
	...
	CUtlDataEnvelope( void* pData, int nBytes, bool ); // takes ownership of pData
	CUtlDataEnvelope( const CUtlDataEnvelope &from );
	CUtlDataEnvelope( CUtlDataEnvelope&& from );
	~CUtlDataEnvelope();
...
public:
	...
	CUtlEnvelope( T* pData, int nElems, bool );
	CUtlEnvelope( const CUtlEnvelope<T> &from );
	CUtlEnvelope( CUtlEnvelope<T>&& from );
...
inline CUtlDataEnvelope::CUtlDataEnvelope( void *pData, int, bool )
{
	m_nBytes = 5; // force delete[] call
	m_pData = static_cast<byte*>( pData );
}
...
inline CUtlDataEnvelope::CUtlDataEnvelope( CUtlDataEnvelope&& from )
	: m_pData( nullptr ), m_nBytes( 0 )
{
	V_swap( from.m_pData, m_pData );
	V_swap( from.m_nBytes, m_nBytes );
}
...
template <typename T>
inline CUtlEnvelope<T>::CUtlEnvelope( T *pData, int nElems, bool )
	: CUtlDataEnvelope( pData, sizeof(T) * nElems, true )
{
}
...
template <typename T>
inline CUtlEnvelope<T>::CUtlEnvelope( CUtlEnvelope<T>&& from )
	: CUtlDataEnvelope( std::forward<CUtlEnvelope<T>>( from ) )
{

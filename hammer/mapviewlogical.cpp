...
#define TIMER_BLINK_INTERVAL	600
...
static constexpr color32 s_pWireColors[LOGICAL_CONN_COLOR_COUNT][LOGICAL_CONN_SELECTION_STATES] =
{
...
static constexpr color32 s_pBrokenWireColor[LOGICAL_CONN_SELECTION_STATES] =
{
...
void CMapViewLogical::AddToRenderLists( CMapClass *pObject )
{
	...
	if ( !pObject->IsGroup() && pObject->IsLogical() )
	{
		...
		Assert( IsValidBox( vecMins, vecMaxs ) );

		if ( IsInClientView( vecMins, vecMaxs ) || !pObject->IsMapClass( MAPCLASS_TYPE( CMapEntity ) ) ||
			static_cast<CMapEntity*>( pObject )->Connections_GetCount() > 0 || static_cast<CMapEntity*>( pObject )->Upstream_GetCount() > 0 )
		{

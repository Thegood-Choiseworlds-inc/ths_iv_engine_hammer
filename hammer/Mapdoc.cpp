...
void CMapDoc::PostloadDocument(const char *pszFileName)
{
	...
	GetMainWnd()->UpdateTextureWindows();
...
void CMapDoc::UpdateAllViews(int nFlags, UpdateBox *ub )
{
	...
	while (p)
	{
		...
		if ( pView && ( !m_bUpdateOnly3D || pView->GetDrawType() > VIEW2D_XZ ) )
		{
...
void CMapDoc::UndoRedo( bool redo )
{
	OnUndoRedo( redo ? ID_EDIT_REDO : ID_EDIT_UNDO );
}
...
void CMapDoc::AllowOnly3DUpdate( bool allow )
{
	m_bUpdateOnly3D = allow;
}

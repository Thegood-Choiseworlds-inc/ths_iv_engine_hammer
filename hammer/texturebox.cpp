...
void CTextureBox::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if (lpDrawItemStruct->itemID != (uint)-1)
	{
		...
		if (pTex != NULL && lpDrawItemStruct->itemState & ODS_SELECTED)
		{
...
void CTextureBox::LoadGraphicList(void)
{
	...
	CUsedTextureList textures;
	if ( GetActiveWorld() )
		GetActiveWorld()->GetUsedTextures( textures );
	InitStorage( textures.Count() + 32, sizeof( PVOID ) );
	...
	{
		char szStr[MAX_PATH];
		tex.pTex->GetShortName( szStr );
		int err = AddString( szStr );
		Assert( ( err != CB_ERR ) && ( err != CB_ERRSPACE ) );
		SetItemDataPtr( nStrCount, tex.pTex );
		nStrCount++;
	}

...
void COP_Entity::UpdateData( int Mode, PVOID pData, bool bCanEdit )
{
	...
	if (Mode == LoadFirstData)
	{
		...
		if ( pEdit->IsClass("worldspawn") || m_bCanEdit == false )
		{
			...
			m_PasteControl.EnableWindow(m_bCanEdit && !kvClipboardClass.IsEmpty() && pEdit->IsClass(kvClipboardClass));
		}
		else
		{
			m_cClasses.EnableWindow(TRUE);
			m_PasteControl.EnableWindow(!bKvClipEmpty && !kvClipboardClass.IsEqual_CaseInsensitive("worldspawn"));
		}
...
void COP_Entity::SetReadOnly(bool bReadOnly)
{
	...
	if ( bReadOnly )
		m_PasteControl.EnableWindow( FALSE );
	else
		m_PasteControl.EnableWindow( !bKvClipEmpty && ( !!V_stricmp( m_cClasses.GetCurrentItem(), "worldspawn" ) != kvClipboardClass.IsEqual_CaseInsensitive( "worldspawn" ) ) );
...
void COP_Entity::OnCopy(void)
{
	...
	m_PasteControl.EnableWindow(TRUE);

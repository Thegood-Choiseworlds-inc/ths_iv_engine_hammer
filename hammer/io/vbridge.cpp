...
constexpr int BRIDGE_DRAW_SEGMENTS_MIN = 8;
constexpr int BRIDGE_DRAW_SEGMENTS_MAX = 64;

constexpr int BRIDGE_DRAW_ARROW_MIN = 4;
constexpr int BRIDGE_DRAW_ARROW_MAX = 16;
...
CBridge::CBridge( CNodeView* p )
{
	...
	SetupVguiTex( m_iTex_Arrow, "editor/arrow" );
...
CBaseNode* CBridge::GetEndNode( const CBaseNode* n ) const
{
...
CBaseNode* CBridge::GetEndNode( const CJack* j ) const
{
	return GetEndNode( j->GetParentNode() );
}

CJack* CBridge::GetEndJack( const CBaseNode* n ) const
{
	if ( !n || ( pN_Src != n && pN_Dst != n ) )
		return nullptr;
...
CJack* CBridge::GetEndJack( const CJack* j ) const
{
	return GetEndJack( j->GetParentNode() );
}

void CBridge::SetTemporaryTarget( const Vector2D& tg )
{
	tmp = tg;
}
							 ...
							 float fraction, bool autosmooth_key1, bool autosmooth_key2, Vector2D& output )
{
	if ( autosmooth_key1 )
	{
		...
		cp1 = start + dir * end.DistTo( start ) * 0.4f;
	}
	if ( autosmooth_key2 )
	{
		...
		cp2 = end + dir * end.DistTo( start ) * 0.4f;
	}
...
void CBridge::VguiDraw( bool bShadow )
{
...
#define BRIDGEHULLEXTRUDE 60.0f
	...
	if ( bounds_max.x < bounds_min.x )
		V_swap( bounds_min.x, bounds_max.x );
	if ( bounds_max.y < bounds_min.y )
		V_swap( bounds_min.y, bounds_max.y );
	bounds_min -= Vector2D( BRIDGEHULLEXTRUDE, BRIDGEHULLEXTRUDE );
	bounds_max += Vector2D( BRIDGEHULLEXTRUDE, BRIDGEHULLEXTRUDE );
	V_swap( bounds_min.y, bounds_max.y );
	if ( !ShouldSimpleDrawObject( pNodeView, pNodeView, bounds_min, bounds_max ) )
		return;

	if ( ji )
		start.x = ji->GetBoundsMax().x;
	if ( jd )
		end.x = jd->GetBoundsMin().x;
	...
	CBaseNode* pN1 = relativeJack_1 ? relativeJack_1->GetParentNode() : nullptr;
	...
	IMaterial* pMatArrow = g_pMatSystemSurface->DrawGetTextureMaterial( m_iTex_Arrow );;
	...
	side *= 15.0f * pNodeView->GetZoomScalar();
	...
	vgui::surface()->DrawSetTexture( m_iTex_Arrow );
	...
	for ( int i = 1; i <= segments; i++ )
	{
		...
		vgui::surface()->DrawLine( append.x, append.y - 1, goal.x, goal.y - 1 );
		vgui::surface()->DrawLine( append.x, append.y, goal.x, goal.y );
		vgui::surface()->DrawLine( append.x, append.y + 1, goal.x, goal.y + 1 );
		append = goal;
	}
}
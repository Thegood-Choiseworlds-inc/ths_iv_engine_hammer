...
bool CToolDisplace::RateLimit()
{
	if ( !m_rateTimer.BLimitReached() )
		return false;
	constexpr uint64 rateUS = 10 * 1000ULL;
	m_rateTimer.SetLimit( rateUS );
	return true;
}
...
bool CToolDisplace::OnMouseMove3D( CMapView3D *pView, UINT nFlags, const Vector2D &vPoint )
{
	...
	if ( m_bRateLimit && ( m_bRMBDown || m_bLMBDown ) && !RateLimit() )
	{
		pView->UpdateView( MAPVIEW_UPDATE_ONLY_3D );
		return true;
	}

	pView->GetMapDoc()->AllowOnly3DUpdate( true );
	...
	pView->GetMapDoc()->AllowOnly3DUpdate( false );

...
void CTextureSystem::ShutDown()
{
	if ( m_pActiveContext )
	{
		...
		auto dest = APP()->GetProfileKeyValues( "Texture Browser" )->FindKey( "Texture History", true );
		while ( auto k = dest->GetFirstSubKey() )
			dest->RemoveSubKey( k );
		history->CopySubkeys( dest );
	}
...
void CTextureSystem::LoadAllGraphicsFiles()
{
	...
	//for (int nConfig = 0; nConfig < Options.configs.GetGameConfigCount(); nConfig++)
	{
		...
		m_pActiveContext->pAllGroup->Sort( nullptr );
	}
}
...
static color24 currentSortColor;
static int __cdecl SortTexturesColor( IEditorTexture* const* elem1, IEditorTexture* const* elem2 )
{
	auto pElem1 = *elem1;
	auto pElem2 = *elem2;
	...
	const bool el1Valid = pElem1->HasValidColorInformation();
	const bool el2Valid = pElem2->HasValidColorInformation();

	if ( !el1Valid && !el2Valid )
		return 0;
	...
	const auto el1Dist = pElem1->ClosestColorDist( currentSortColor );
	const auto el2Dist = pElem2->ClosestColorDist( currentSortColor );
...
void CTextureSystem::ReSort( const color24* color )
{
	if ( m_pActiveGroup )
		m_pActiveGroup->Sort( color );
}

void CTextureSystem::CalcColorInfoForAllMaterials()
{
	if ( m_pActiveGroup )
		m_pActiveGroup->CalcColorInfoForAllMaterials();
}
...
//-----------------------------------------------------------------------------
void CTextureGroup::Sort( const color24* color )
{
	if ( color )
		currentSortColor = *color;
	m_Textures.Sort( color ? SortTexturesColor : SortTexturesProc );
...
void CTextureGroup::CalcColorInfoForAllMaterials()
{
	for ( auto tex : m_Textures )
	{
		if ( tex->IsDummy() )
			continue;
		if ( tex->Load() )
			static_cast<CMaterial*>( tex )->TryLoadColorData();
	}
}